...
class TClust(ClusterMixin, BaseEstimator, TransformerMixin):
    ...
    def fit(self, X, y=None):
        ...
        for j in range(self.niter):
            ...
            if self.iter.obj > self.best_iter.obj:
                print('(%d/%d) New iteration is better! Old obj: %.2f; New obj: %.2f' % (j+1, self.niter, self.best_iter.obj, self.iter.obj))
                self.best_iter.update(obj=self.iter.obj, labels_=self.iter.labels_, csize=self.iter.csize, cw=self.iter.cw,
        ...
        for k in range(self.k):
            if self.iter.csize[k] > self.zero_tol:  # if cluster size is > 0
                ...
                if not self.tk:
                    ...
                    self.iter.lambd[k] = np.mean(np.sum(np.matmul(self.iter.z_ij[:, k].T, X_c**2) / self.iter.csize[k], axis=0))
    ...
    def findClusterlabels_(self, X):
        ...
        if self.opt == 'hard':
            ...
            self.iter.code = 2 * np.all(old_labels_ == self.iter.labels_)  # setting the code - parameter, signaling whether the labels_nment is the same than the previous --- is the only stopping rule implemented
    ...
    def restr2_eigenv(self, autovalues, ni_ini, factor_e, zero_tol):
        ...
        if n == 0:  # TODO: seems fixed now...
            ...
            print('this should break:', max(d[nis>0])) # "zero-size array to reduction operation maximum which has no identity"
...
def dmnorm_tk(x, mu, lambd):
    ...
    b = (np.matmul(np.ones((x.shape[0], 1)), mu) - x) ** 2
